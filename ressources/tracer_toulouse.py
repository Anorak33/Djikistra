...
def tracer_route_toulouse(fichier_graphml: str, route: list):
    ...
    figure, axes = ox.plot_graph(sous_graphe_connexe, show=False, close=False,
                                 node_size=0, edge_color="#999999", edge_linewidth=0.5)
    # utilitaire pour obtenir les géométries d'arêtes correspondant à la route
    try:
        route_arcs_gdfs = ox.utils_graph.route_to_gdfs(sous_graphe_connexe, route, nodes=False, edges=True)
        route_arcs_gdfs.plot(ax=axes, linewidth=3, edgecolor="red")
    except Exception:
        # tracer la polyline des nœuds si route_to_gdfs non disponible
        coordonnees = [(sous_graphe_connexe.nodes[n]["y"], sous_graphe_connexe.nodes[n]["x"]) for n in route]
    ...
    depart = route[0]
    arrivee = route[-1]
    depart_longitude, depart_latitude = sous_graphe_connexe.nodes[depart]["x"], sous_graphe_connexe.nodes[depart]["y"]
    arrive_longitude, arrive_latitude = sous_graphe_connexe.nodes[arrivee]["x"], sous_graphe_connexe.nodes[arrivee]["y"]

    # afficher les extrémités de la route sur la figure différemment (ex. vert pour départ, bleu pour arrivée)
